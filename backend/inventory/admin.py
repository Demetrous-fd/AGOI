...
class InstanceAdmin(SimpleHistoryAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        inventory_numbers = form.cleaned_data["inventory_numbers"].split("\n")
        inventory_numbers = [value.strip() for value in inventory_numbers]
        ...
        del obj["inventory_number"]
        for index, number in enumerate(inventory_numbers):
            if index == 0:
                new_obj = self.model(pk=obj_pk, **obj, inventory_number=number)
            else:
                new_obj = self.model(**obj, inventory_number=number)
