...
class Instance(models.Model):
    ...
    inventory_number = models.CharField(
        max_length=32, unique=True, db_index=True, verbose_name="Инвентарный номер")
...
class ReportItem(models.Model):
    ...
    report = models.ForeignKey(Report, on_delete=models.CASCADE, db_index=True, related_name="items")


class ReportScannedItem(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    instance = models.ForeignKey(Instance, on_delete=models.CASCADE)
    status = models.CharField(max_length=8, choices=[
        ("ok", "OK"),
        ("error", "Не был в списке инвентаризации"),
    ], default="ok")
    report = models.ForeignKey(Report, on_delete=models.CASCADE, db_index=True, related_name="scanned_items")
